...
#1.QGAN 训练阶段：使用 SurfaceCodeDataset 作为真实数据，训练生成器和判别器。
#生成器学会生成类似于真实综合征错误（tensor_syndrome）的数据。

#2.Transformer 微调阶段：使用 QGAN 生成的综合征错误数据（tensor_syndrome）作为输入，训练 Transformer 模型。
#Transformer 的目标是预测逻辑错误状态（logical_state）。

#混合方案优势：
#QGAN 学习生成高质量的综合征错误数据，使得 Transformer 模型可以利用扩展后的数据进行更全面的训练。
import torch
...
class QuantumGenerator(nn.Module):
    def __init__(self, n_qubits=5, q_depth=6, n_generators=1):
        super().__init__()
...
dataset = SurfaceCodeDataset(noise_model)
...
# ------------------------------
num_epochs_qgan = 5
generated_syndrome_data = []  # 保存生成的综合征错误数据
